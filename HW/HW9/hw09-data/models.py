...
def linear_regression(X, Y, Xs_test, Ys_test):
    ...
    trueX = poly.fit_transform(X)
    INFO = np.linalg.inv(trueX.T @ trueX) @ trueX.T @ Y
    ANS = [np.mean(np.sqrt(np.sum((poly.fit_transform(X_test) @ INFO - Y_test) ** 2, axis=1))) for X_test, Y_test in zip(Xs_test, Ys_test)]
    return ANS
    #return 0
...
def neural_network(X, Y, Xs_test, Ys_test):
    ...
    ANS = [mse.eval({x: X_test, y: Y_test}) for X_test, Y_test in zip(Xs_test, Ys_test)]
    return ANS
    #return 0

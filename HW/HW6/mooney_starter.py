...
class Mooney(object):
	...
	def solve_for_variance(self):
		...
		# DONE
		laaaambda = 0.00001
		A = inv(sqrtm(self.C_xx+laaaambda*np.eye(self.x_dim)))
		B = inv(sqrtm(self.C_yy+laaaambda*np.eye(self.y_dim)))
		...
		plt.show()
		#singular_values&vectors
		singular_values = s
		singular_vectors = u
		eigen_values=np.sqrt(singular_values)
		eigen_vectors=np.sqrt(singular_vectors)
	...
	def ridge_regression(self):
		w_ridge = []
		laaaambda = 0.00001
		A = np.matmul(self.X_ridge.T,self.X_ridge)+laaaambda*np.eye(np.matmul(self.X_ridge.T,self.X_ridge).shape[0])
		p1 = LA.inv(A)
		for i in range(self.y_dim):
			# TODO: IMPLEMENT RIDGE REGRESSION DONE
			#w_ridge.append(np.zeros((self.X_ridge.shape[1],)))
			p2 = np.matmul(self.Y_ridge.T[i],self.X_ridge)
			temp = np.matmul(p2,p1)
			w_ridge.append(temp)
...
def eigen_face(self,eig_val,eig_vec):
    	#u,s,d = svd(C)
		#singular_vectors = u
		eigen_vectoraaaaaaaaaa = self.ss_x.inverse_transform(eig_vec[:,0])
		self.plot_image(eigen_vectoraaaaaaaaaa)
...
if __name__ == '__main__':
	...
	mooney.eigen_face(eig_val,eig_vec)

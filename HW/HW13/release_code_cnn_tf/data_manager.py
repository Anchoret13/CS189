...
class data_manager(object):
    ...
    def load_train_set(self):
        ...
        self.feature = np.asarray(self.feature, dtype = np.float)
    ...
    def load_validation_set(self):
        ...
        self.val_label = []
        for i in range(len(self.train_data)):
            self.val_feature.append(self.train_data[i]['features'])
            self.val_label.append(self.train_data[i]['label'])
        self.val_feature = np.asarray(self.feature, dtype = np.float)
        self.val_label = np.asarray(self.label,dtype = np.float)

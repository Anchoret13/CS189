...
class Viz_Feat(object):
    ...
    def vizualize_features(self,net):
        ...
        j=0
        for img in images:
            val_feature = np.asarray([self.val_data[img]['features']], dtype = np.float)
            val_lable = np.asarray([self.val_data[img]['label']], dtype = np.float)
            feed_dict = {net.images: val_feature, net.labels:val_lable}
